...
class FromAccountSelectorWidget extends StatelessWidget {
  final List<AccountEntity> accounts;
  final AccountEntity selectedAccount;

  const FromAccountSelectorWidget({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CurrencyExchangeBloc, CurrencyExchangeState>(
      ...
      builder: (context, state) {
        final currentAccount = state.fromAccount ?? selectedAccount;
        return DropdownSelectorWidget<AccountEntity>(
          title: context.translate('from_account'),
          subtitle: context.translate('choose_account_to_exchange_from'),
          ...
          onChanged: (AccountEntity? newAccount) {
            if (newAccount != null) {
              context.read<CurrencyExchangeBloc>().add(
                SelectFromAccountEvent(account: newAccount),
              );
            }
          },
        );
