...
class FromCurrencySelectorWidget extends StatelessWidget {
  final AccountEntity? selectedAccount;

  const FromCurrencySelectorWidget({super.key, required this.selectedAccount});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<CurrencyExchangeBloc, CurrencyExchangeState>(
      ...
      builder: (context, state) {
        final currentCurrency = currencies[state.fromCurrencyIndex];
        return DropdownSelectorWidget<CurrencyBalanceEntity>(
          title: context.translate('from_currency'),
          subtitle: context.translate('choose_currency_to_exchange_from'),
          ...
          onChanged: (CurrencyBalanceEntity? newCurrency) {
            if (newCurrency != null) {
              ...
              context.read<CurrencyExchangeBloc>().add(
                SelectFromCurrencyEvent(currencyIndex: index),
              );
            }
          },
        );
